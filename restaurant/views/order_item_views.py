...
from restaurant.serializers.order_item_serializer import LiveOrderSerializer
from restaurant.serializers.order_item_serializer import (AddressSerializer,
                                                          OrderSerializer,
                                                          PaymentSerializer)
...
class AddToCartView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        if subdomain is None or outletID is None or itemID is None or locationID is None:
        ...
        if order_qs.exists():
            ...
            order = Order.objects.create(restaurant=restaurant, outlet=outlet, user=request.user,
                                         location=location, ordered_date=ordered_date)
            order.items.add(order_item)
            return Response(status=HTTP_200_OK)
